...
class WindowAttention(nn.Module):
    ...
    def __init__(self, dim, window_size, num_heads, qkv_bias=True, qk_scale=None, attn_drop=0., proj_drop=0.):
        ...
        self.qk1v = nn.Conv2d(dim, dim * 3, kernel_size=1, bias=True)
        self.qk2v = nn.Conv2d(dim, dim * 3, kernel_size=1, bias=True)
    ...
    def forward(self, x, mask=None):
        ...
        temp1 = self.qkv_dwconv(self.qk1v(x.permute(2,0,1))).permute(1,2,0)
        temp2 = self.qkv_dwconv(self.qk2v(x.permute(2,0,1))).permute(1,2,0)
        qk1v = temp1.reshape(B_, N, 3, self.num_heads, C // self.num_heads).permute(2, 0, 3, 1, 4)
        qk2v = temp2.reshape(B_, N, 3, self.num_heads, C // self.num_heads).permute(2, 0, 3, 1, 4)
        q, k1, v = qk1v[0], qk1v[1], qk1v[2]  # make torchscript happy (cannot use tensor as tuple)
        q, k2, v = qk2v[0], qk2v[1], qk2v[2]
        k=torch.add(k1,k2)
        q = q * self.scale
